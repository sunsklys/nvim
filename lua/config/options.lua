-- 启动期 env 注入已抽出到 lua/config/env.lua（luarocks + LG_CONFIG_FILE + GIT_CONFIG_*）
-- 在 options.lua 顶部 require，保证所有插件 spec 加载前 env 已就位
require("config.env")
...
vim.opt.scrolloff = 3
-- 删 BufEnter：scrolloff 只依赖 win height，同窗口内切 buffer 不改 height，BufEnter 是冗余触发
